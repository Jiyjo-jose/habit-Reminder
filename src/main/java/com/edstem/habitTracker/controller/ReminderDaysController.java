...
public class ReminderDaysController {
    ...
    public ResponseEntity<?> addReminderDaysToHabit(
        ...
        try {
            reminderDaysService.addReminderDaysToHabit(habitId, addReminderDaysRequest.getReminderDays());
            return ResponseEntity.ok("Reminder days added successfully");
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error adding reminder days");
        }
    }
    ...
    public ResponseEntity<String> completeReminderDay(
            @PathVariable Long habitId,
            @PathVariable Long reminderDayId) {
    ...
    public ResponseEntity<List<ReminderDays>> getCompletedReminderDays(
            @PathVariable Long habitId) {
        List<ReminderDays> completedReminderDays = reminderDaysService.getCompletedReminderDays(habitId);
        return ResponseEntity.ok(completedReminderDays);
    }
    @GetMapping("/{habitId}/incompleteReminderDays")
    public ResponseEntity<List<ReminderDays>> getIncompleteReminderDays(@PathVariable Long habitId) {
        List<ReminderDays> incompleteReminderDays = reminderDaysService.getIncompleteReminderDays(habitId);
...
//    @PutMapping("/{habitId}/editReminderDays")
//    public ResponseEntity<String> editReminderDays(
//            @PathVariable Long habitId,
//            @RequestBody List<ReminderDays> updatedReminderDays) {
//        reminderDaysService.editReminderDays(habitId, updatedReminderDays);
//        return ResponseEntity.ok("Reminder days edited successfully");
//    }
